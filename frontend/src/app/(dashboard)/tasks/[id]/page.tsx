...
export default function TaskDetailPage() {
  ...
  const reassignMutation = useMutation({
    mutationFn: (assigneeId: number) =>
      tasksService.updateTask(taskId, { assignee_id: assigneeId }),
    onSuccess: () => {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="rounded-2xl border bg-card p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                {/* Reassign button — only for the assigner, only on pending tasks */}
                {isAssigner && !isCompleted && !reassigning && (
                  <button
                    onClick={() => {
                      setReassigning(true);
                      setNewAssigneeId(String(task.assignee_id));
                    }}
              ...
              {reassigning ? (
                <div className="space-y-2">
                  <select
                    ...
                    className="w-full px-3 py-2 rounded-xl border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition"
                  >
                    <option value="">Select new assignee...</option>
                    {users.map((u) => (
                      <option key={u.id} value={u.id}>
                        {u.full_name}{u.id === task.assignee_id ? " (current)" : ""}
                      </option>
                    ))}
                  </select>
                  <div className="flex gap-2">
                    <button
                      onClick={() => {
                        if (!newAssigneeId || Number(newAssigneeId) === task.assignee_id) {
                          setReassigning(false);
                          return;
                        }
                        reassignMutation.mutate(Number(newAssigneeId));
                      }}
                      disabled={reassignMutation.isPending || !newAssigneeId}
                      className="flex-1 flex items-center justify-center gap-1.5 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-medium transition disabled:opacity-60"
                    >
                    ...
                    <button
                      onClick={() => { setReassigning(false); setNewAssigneeId(""); }}
                      className="flex-1 flex items-center justify-center gap-1.5 py-1.5 border rounded-xl text-xs font-medium hover:bg-accent transition"
                    >
